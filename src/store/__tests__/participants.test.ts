...
describe('participants store', () => {
  beforeEach(() => {
    ...
    const { removeParticipant, participants: storeParticipants } = useParticipantsStore.getState()
  ...
  it('updates a participant', () => {
    const { addParticipant, updateParticipant } = useParticipantsStore.getState()
    ...
    expect(llmParticipant.systemPrompt).toBe('You are a super helpful assistant')
  ...
  it('sets the active participant', () => {
    ...
    // Get the updated state
    const activeParticipantId = useParticipantsStore.getState().activeParticipantId
    ...
    // Get the updated state
    const finalActiveParticipantId = useParticipantsStore.getState().activeParticipantId
  ...
  it('reorders participants', () => {
    const { addParticipant, reorderParticipants } = useParticipantsStore.getState()
    ...
    vi.mocked(crypto.randomUUID).mockReturnValueOnce('test-uuid-2' as `${string}-${string}-${string}-${string}-${string}`)
  ...
  it('clears the active participant when removing the active participant', () => {
    const { addParticipant, setActiveParticipant, removeParticipant } = useParticipantsStore.getState()
