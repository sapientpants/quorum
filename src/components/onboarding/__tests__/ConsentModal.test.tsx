...
describe('ConsentModal', () => {
  ...
  it('renders the modal with title and content', () => {
    ...
    expect(screen.getByText(/Please review our privacy policies/)).toBeInTheDocument()
  })

  it('renders storage options with local storage selected by default', () => {
    render(<ConsentModal {...mockProps} />)
    
    const localStorageRadio = screen.getByLabelText('Local Storage') as HTMLInputElement
    const sessionStorageRadio = screen.getByLabelText('Session Storage') as HTMLInputElement
    const noStorageRadio = screen.getByLabelText('No Storage') as HTMLInputElement
  ...
  it('allows changing storage option', () => {
    ...
    expect((screen.getByLabelText('Local Storage') as HTMLInputElement).checked).toBe(false)
  })

  it('renders confirmation checkbox unchecked by default', () => {
    render(<ConsentModal {...mockProps} />)
    
    const checkbox = screen.getByLabelText(/I acknowledge and agree/) as HTMLInputElement
