            ...
            {activeTab === 'basic' && (
              <div className="space-y-6">
                <div>
                  ...
                  {errors.temperature && (
                    <p className="text-error text-sm mt-1">{errors.temperature.message}</p>
                  )}
                  <p className="text-sm mt-3 text-base-content/70">
                    Controls randomness. Lower values make the model more deterministic and focused,
                    while higher values make output more random and creative.
                  </p>
                </div>
                ...
                <div>
                  ...
                  />
                  {errors.maxTokens && (
                    <p className="text-error text-sm mt-1">{errors.maxTokens.message}</p>
                  )}
                  <p className="text-sm mt-1 text-base-content/70">
                    The maximum number of tokens the model will generate. 1 token is about 4 characters.
                    A typical sentence is 15-20 tokens.
                  </p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'experimental' && (
              <div className="space-y-6">
                <div>
                  ...
                  />
                  {errors.topP && (
                    <p className="text-error text-sm mt-1">{errors.topP.message}</p>
                  )}
                  <p className="text-sm mt-1 text-base-content/70">
                    Controls diversity via nucleus sampling. A value of 0.9 means the model considers 
                    the top 90% most probable tokens. Lower values increase focus, higher values increase diversity.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    {errors.presencePenalty && (
                      <p className="text-error text-sm mt-1">{errors.presencePenalty.message}</p>
                    )}
                    <p className="text-sm mt-1 text-base-content/70">
                  ...
                  <div>
                    ...
                    {errors.frequencyPenalty && (
                      <p className="text-error text-sm mt-1">{errors.frequencyPenalty.message}</p>
                    )}
                    <p className="text-sm mt-1 text-base-content/70">
