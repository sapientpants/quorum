...
export function RoundTable({ 
  ...
  useEffect(() => {
    ...
    if (activeParticipantId && participants.length > 1) {
      const thinkingParticipants = participants.filter(p => 
        p.id !== activeParticipantId && p.type === 'llm'
      )
      
      if (thinkingParticipants.length) {
        const randomIndex = Math.floor(Math.random() * thinkingParticipants.length)
  ...
  return (
    <div className={`${className} flex flex-col items-center`}>
      <div className="text-center mb-4">
        <h2 className="text-xl font-semibold">Round Table</h2>
        <p className="text-sm opacity-70">Click on a participant to view details</p>
      </div>
      ...
      >
        ...
        >
          <div className="absolute inset-0 flex items-center justify-center">
            <Icon icon="solar:users-group-rounded-broken" className="w-10 h-10 opacity-30" />
          </div>
        </div>
                  ...
                  {activeParticipant.type === 'llm' && (
                    <>
                      <div className="text-sm opacity-70 mt-1">
                        <span>{activeParticipant.provider}</span> • <span>{activeParticipant.model}</span>
                      </div>
                      {activeParticipant.roleDescription && (
                        <div className="text-sm mt-2">{activeParticipant.roleDescription}</div>
                      )}
                    </>
                  )}
                </>
