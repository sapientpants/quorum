...
function TemplateForm({ initialData, onCancel, onSuccess }: TemplateFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="form-control">
        ...
        {errors.participantIds && (
          <label className="label">
            <span className="label-text-alt text-error">{errors.participantIds.message}</span>
          </label>
        )}
        ...
        {participants.length === 0 && (
          <div className="alert alert-warning mt-2">
            ...
            <span>{t('No participants available. Create participants first.')}</span>
          </div>
        )}
      </div>
      ...
      <div className="form-control">
        <label className="label">
          <span className="label-text font-medium">{t('Default Conversation Starter (Optional)')}</span>
        </label>
        <textarea
          className="textarea textarea-bordered h-24"
          placeholder={t('Enter a message to start the conversation with')}
          {...register('defaultConversationStarter')}
        />
        <label className="label">
          <span className="label-text-alt opacity-70">
            {t('This message will be automatically sent when using this template')}
          </span>
        </label>
      </div>
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        >
          {isSubmitting ? (
            ...
            initialData ? t('Update Template') : t('Create Template')
          )}
        </button>
      </div>
    </form>
