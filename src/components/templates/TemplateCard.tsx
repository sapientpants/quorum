...
function TemplateCard({ template, onUse, onEdit, onDelete, onShare }: TemplateCardProps) {
    ...
    <div className="card bg-base-100 shadow-xl hover:shadow-2xl transition-all duration-300">
      <div className="card-body">
        <div className="flex justify-between items-start">
          ...
          <div className="dropdown dropdown-end">
            ...
            {isMenuOpen && (
              ...
              >
                <li><a onClick={() => onUse(template.id)}>{t('Use Template')}</a></li>
                <li><a onClick={() => onEdit(template.id)}>{t('Edit')}</a></li>
                {onShare && (
                  <li><a onClick={() => onShare(template.id)}>{t('Share')}</a></li>
                )}
                <li>
                  <a 
            ...
            <div className="flex -space-x-2">
              ...
              {templateParticipants.length > 5 && (
                <div className="avatar placeholder">
                  <div className="bg-neutral text-neutral-content w-8 rounded-full">
                    <span className="text-xs">+{templateParticipants.length - 5}</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="card-actions justify-between items-center mt-4">
          <span className="text-xs opacity-50">{t('Updated')}: {formattedDate}</span>
          <button 
