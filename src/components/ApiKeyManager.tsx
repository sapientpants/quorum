...
export function ApiKeyManager({ 
  ...
  const [statusMessages, setStatusMessages] = useState<Record<LLMProviderId, { type: 'success' | 'error', message: string } | null>>({
    openai: null,
    anthropic: null,
    google: null,
    grok: null
  ...
  function handleApiKeyChange(provider: LLMProviderId, value: string) {
    ...
    if (value) {
      ...
      setStatusMessages(prev => ({
        ...prev,
        [provider]: { type: 'success', message: t('settings.apiKeyManager.saved') }
      }))
          ...
          <div key={provider.id} className="space-y-2">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <Label htmlFor={`${provider.id}-api-key`} className="text-base font-medium">
              ...
              {statusMessages[provider.id] && (
                <span className={`text-sm ${statusMessages[provider.id]?.type === 'success' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
            ...
            <div className="relative">
              <Input
                ...
                onChange={(e) => handleApiKeyChange(provider.id as LLMProviderId, e.target.value)}
                placeholder={providerInfo[provider.id as LLMProviderId]?.placeholder}
              ...
              <Button
                ...
                onClick={() => toggleKeyVisibility(provider.id as LLMProviderId)}
                aria-label={isVisible[provider.id] ? t('settings.apiKeyManager.hideKey') : t('settings.apiKeyManager.showKey')}
              >
                <Icon 
                  icon={isVisible[provider.id] ? "solar:eye-closed-linear" : "solar:eye-linear"} 
                  className="h-4 w-4" 
                />
              </Button>
            </div>
