...
describe('ApiKeySetup', () => {
  it('renders correctly', () => {
    ...
    expect(screen.getByText('How to get an OpenAI API key')).toBeInTheDocument()
    expect(screen.getByText('How to get an Anthropic API key')).toBeInTheDocument()
    expect(screen.getByText('How to get a Grok API key')).toBeInTheDocument()
    expect(screen.getByText('How to get a Google AI API key')).toBeInTheDocument()
  })
  ...
  it('validates key format for each provider', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('sk-...'), { target: { value: 'invalid-key' } })
    fireEvent.blur(screen.getByPlaceholderText('sk-...'))
    await waitFor(() => {
      expect(screen.getByTestId('openai-error')).toBeInTheDocument()
      expect(screen.getByTestId('openai-error')).toHaveTextContent('Invalid API key')
    })
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('anthropic-error')).toHaveTextContent('Invalid API key')
    })
  })
  ...
  it('clears errors when user starts typing', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('sk-...'), { target: { value: 'sk-' } })
    fireEvent.blur(screen.getByPlaceholderText('sk-...'))
    await waitFor(() => {
