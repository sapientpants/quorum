  ...
  useTranslation: () => ({
    t: (key: string) => {
      const translations: { [key: string]: string } = {
        "settings.apiKeyManager.openaiDescription":
          "Required for GPT-4o and GPT-4.5 models",
        "settings.apiKeyManager.anthropicDescription":
          "Required for Claude models",
        "settings.apiKeyManager.googleDescription":
...
describe("ApiKeyManager", () => {
  ...
  it("uses the provided storage option", () => {
    ...
    expect(mockedSaveApiKeys).toHaveBeenCalledWith(expect.any(Array), {
      storage: "session",
    });
  });
});
