...
import {
  ErrorContext,
  ErrorContextType,
} from "../../contexts/contexts/ErrorContextDefinition";
  ...
  useTranslation: () => ({
    t: (key: string) => {
      const translations: { [key: string]: string } = {
        ...
        "errors.suggestions.reduceComplexity":
...
describe("ApiErrorModal", () => {
  ...
  it("renders authentication error correctly", () => {
    ...
    expect(
      screen.getByText("Try regenerating your API key"),
    ).toBeInTheDocument();
    expect(screen.getByText("Verify your account status")).toBeInTheDocument();

    // Check technical details
    expect(
      screen.getByText(/errors.technicalDetails.error: Invalid API key/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/errors.technicalDetails.type: authentication/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/errors.technicalDetails.status: 401/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/errors.technicalDetails.provider: openai/),
    ).toBeInTheDocument();
  ...
  it("renders rate limit error correctly", () => {
    ...
    expect(
      screen.getByText("Consider upgrading your account"),
    ).toBeInTheDocument();
