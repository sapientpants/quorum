...
describe('SettingsPanel', () => {
  ...
  it('renders the settings panel with correct values', () => {
    ...
    expect(screen.getByText('Streaming shows responses as they are generated')).toBeInTheDocument()
    
    // Check that the streaming toggle is checked
    const streamingToggle = screen.getByRole('checkbox', { name: 'Enable streaming' })
  ...
  it('does not render streaming toggle when streaming is not supported', () => {
    ...
    expect(screen.queryByText('Streaming shows responses as they are generated')).not.toBeInTheDocument()
  })
