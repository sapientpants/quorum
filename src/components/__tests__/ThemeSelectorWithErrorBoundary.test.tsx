...
import {
  ThemeContext,
  ThemeContextType,
} from "../../contexts/contexts/ThemeContextDefinition";
  ...
  default: ({
    ...
    return (
      <div data-testid="error-boundary">
        {children}
        <div data-testid="fallback" style={{ display: "none" }}>
          {fallback}
        </div>
      </div>
    );
...
describe("ThemeSelectorWithErrorBoundary", () => {
  ...
  it("provides a fallback component to the ErrorBoundary", () => {
    ...
    expect(
      fallback.querySelector('[data-testid="button"]'),
    ).toBeInTheDocument();
  });
  ...
  it("provides an onError callback to the ErrorBoundary", () => {
    ...
    const onErrorFn = ErrorBoundaryMock.onError as unknown as (
      error: Error,
    ) => void;
    onErrorFn(error);
