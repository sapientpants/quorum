    ...
    case "openai":
      ...
      if (
        ...
        return {
          isValid: false,
          message:
            'Invalid OpenAI API key format. Should start with "sk-" (51 characters) or "sk-proj-" (164 characters)',
        };
      }
      break;
    case "anthropic":
      ...
      if (!key.startsWith("sk-ant-") || key.length < 40) {
        return {
          isValid: false,
          message:
            'Invalid Anthropic API key format. Should start with "sk-ant-" and be at least 40 characters',
        };
      }
      break;
    case "grok":
      ...
      if (!key.startsWith("grok-") || key.length < 40) {
        return {
          isValid: false,
          message:
            'Invalid Grok API key format. Should start with "grok-" and be at least 40 characters',
        };
      }
      break;
    case "google":
      ...
      if (key.length < 40) {
        return {
          isValid: false,
          message:
            "Invalid Google AI API key format. Should be at least 40 characters",
        };
      }
      break;
    default:
