...
import type { LLMSettings, StreamingOptions, ProviderCapabilities } from '../../../../types/llm'
...
describe('BaseClient', () => {
  ...
  it('handles different types of errors', () => {
    ...
    expect(() => handleApiError(contentError)).toThrow('filtered due to content policy')
  ...
  it('handles errors in streamMessage', async () => {
    ...
    for await (const chunk of client.streamMessage(messages, 'valid-key', 'test-model')) {
