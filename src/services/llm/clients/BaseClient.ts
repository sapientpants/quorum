...
export abstract class BaseClient implements LLMClient {
  ...
  protected handleApiError(error: unknown): never {
    ...
    if (error instanceof LLMError) {
      ...
      } else if (status === 400 && (errorBody.error?.type === 'content_filter' || errorBody.error?.code === 'content_filter')) {
        llmError = new LLMError(
   ...
   */
  public abstract chat(messages: LLMChatMessage[], config?: Partial<LLMConfig>): Promise<LLMResponse>
