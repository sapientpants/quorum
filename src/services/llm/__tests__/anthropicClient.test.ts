...
describe('AnthropicClient', () => {
  ...
  it('sends settings correctly', async () => {
    ...
    await client.sendMessage(messages, 'test-key', 'claude-3.7-sonnet', settings)
  ...
  it('handles network errors correctly', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key')).rejects.toThrow('An error occurred while calling the Anthropic API: Network error')
  })
  ...
  it('handles empty response errors correctly', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key')).rejects.toThrow('No response from Anthropic')
  })
  ...
  if (typeof ReadableStream !== 'undefined' && typeof TextDecoder !== 'undefined') {
    it('uses streaming when streamingOptions are provided', async () => {
      ...
      const response = await client.sendMessage(messages, 'test-key', 'claude-3.7-sonnet', undefined, streamingOptions)
      ...
      expect(streamingOptions.onComplete).toHaveBeenCalledWith('Hello from Claude')
    ...
    it('calls onError when streaming fails', async () => {
      ...
      await expect(client.sendMessage(messages, 'test-key', 'claude-3.7-sonnet', undefined, streamingOptions)).rejects.toThrow('An error occurred while calling the Anthropic API')
