...
describe('GrokClient', () => {
  ...
  it('throws an error when sending a message without an API key', async () => {
    ...
    await expect(client.sendMessage(messages, '')).rejects.toThrow('Invalid or missing API key')
  })
  ...
  it('throws an error when using an unsupported model', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key', 'unsupported-model')).rejects.toThrow('Model unsupported-model is not available')
  })
  ...
  it('sends settings correctly', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        body: expect.stringContaining('"temperature":0.7,"max_tokens":100,"top_p":0.9')
      })
    )
  })
  ...
  it('handles API errors correctly', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key')).rejects.toThrow('Invalid or expired API key')
  })
  ...
  it('handles network errors correctly', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key')).rejects.toThrow('An error occurred while calling the Grok API: Network error')
  })
  ...
  it('handles rate limit errors correctly', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key')).rejects.toThrow('Too many requests')
  })
  ...
  it('handles model not found errors correctly', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key')).rejects.toThrow('The requested model is not available')
  })
  ...
  it('handles empty response errors correctly', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key')).rejects.toThrow('No response from Grok')
  })
  ...
  if (typeof ReadableStream !== 'undefined' && typeof TextDecoder !== 'undefined') {
    it('uses streaming when streamingOptions are provided', async () => {
      ...
      mockReader.read.mockImplementation(() => {
        if (callCount < chunks.length) {
          ...
          return Promise.resolve({
            ...
            value: new TextEncoder().encode(`data: {"choices":[{"delta":${JSON.stringify(chunk)}}]}\n\n`)
          })
      ...
      const response = await client.sendMessage(messages, 'test-key', 'grok-3', undefined, streamingOptions)
      ...
      expect(streamingOptions.onComplete).toHaveBeenCalledWith('Hello from Grok')
    ...
    it('calls onError when streaming fails', async () => {
      ...
      await expect(client.sendMessage(messages, 'test-key', 'grok-3', undefined, streamingOptions)).rejects.toThrow('An error occurred while calling the Grok API')
