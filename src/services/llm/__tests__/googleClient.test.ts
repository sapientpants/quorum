...
describe('GoogleClient', () => {
  ...
  it('throws an error when sending a message without an API key', async () => {
    ...
    await expect(client.sendMessage(messages, '', 'gemini-2.0-pro')).rejects.toThrow('API key is required')
  })
  ...
  it('throws an error when using an unsupported model', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key', 'unsupported-model')).rejects.toThrow('Model unsupported-model is not supported')
  })
  ...
  it('logs messages, settings, and streaming options when sending a message', async () => {
    ...
    await client.sendMessage(messages, 'test-key', 'gemini-2.0-pro', settings, streamingOptions)
    
    expect(consoleSpy).toHaveBeenCalledWith('Using settings:', settings)
    expect(consoleSpy).toHaveBeenCalledWith('Using streaming options:', streamingOptions)
    expect(consoleSpy).toHaveBeenCalledWith('Processing messages:', messages.length)
  })
  ...
  it('returns a placeholder response when sending a message', async () => {
    ...
    const response = await client.sendMessage(messages, 'test-key', 'gemini-2.0-pro')
  ...
  it('wraps errors in GoogleError', async () => {
    ...
    await expect(client.sendMessage(messages, 'test-key', 'gemini-2.0-pro')).rejects.toThrow('Failed to send message to Google API')
  })
})
