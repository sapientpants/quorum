...
vi.mock('../../components/ApiKeyManager', () => ({
  default: ({ onApiKeyChange, storageOption }: { 
...
describe('Settings', () => {
  ...
  it('handles key storage preference changes', async () => {
    ...
    expect(updatePreferences).toHaveBeenCalledWith({ keyStoragePreference: 'session' })
  ...
  it.skip('exports data when export button is clicked', async () => {
    ...
    localStorageMock.key = vi.fn((i) => i === 0 ? 'test-key' : i === 1 ? 'test-json' : null)
